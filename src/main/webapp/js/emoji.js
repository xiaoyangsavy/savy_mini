...
var ImgIputHandler={
    ...
    Init:function(){
        ...
        // });
        // $(".board-comment-textarea").focus(function(){
        ...
        // });
        // if($(".emoji-list-div").children().length==0) {
            for (var i = 0; i < ImgIputHandler.facePath.length; i++) {//初始化表情包
                $(".emoji-list-div").append("<img title=\"" + ImgIputHandler.facePath[i].faceName + "\" src=\"../images/emoji/" + ImgIputHandler.facePath[i].facePath + "\" />");
            }
            $(".emoji-list-div>img").click(function () {//表情点击事件

                // isShowImg = false;
                // $(this).parent().animate({marginTop:"0px"},300);
                ImgIputHandler.insertAtCursor($(".board-comment-textarea")[0], "[" + $(this).attr("title") + "]");
            });
        // }
        $(".emoji-button").click(function(){
            if(isShowImg==false){
                isShowImg=true;
                $("#emoji-list-div").css("display", "");
                // $(this).parent().prev().animate({marginTop:"-125px"},300);
            }else{
                isShowImg=false;
                $("#emoji-list-div").css("display", "none");
